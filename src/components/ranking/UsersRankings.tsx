    ...
    <Box display="flex" flexDirection="column" gap={2}>
      <Box>
        <Box display="flex" flexDirection="row" gap={2}>
          {/* Activity */}
          <Box flex={1}>
            <Typography variant="h6" component="span" sx={{ color: "text.secondary", ml: 4 }}>
              GeekLink Activities
            </Typography>
            ...
            {Array.from({ length: 5 - activity.length }).map((_, index) => (
              <Box key={`empty-${index}`} p={2} borderRadius={1}>
              </Box>
            ))}
          </Box>
          ...
          <Box flex={1}>
            ...
            {[{ data: contribution, type: "contribution" }].map((ranking, index) => (
              <Box key={index} p={2} borderRadius={1}>
                <UsersRanking data={ranking.data} type={ranking.type} />
              </Box>
            ))}
