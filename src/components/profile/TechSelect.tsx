  ...
  return (
    <div>
      ...
      <button
        ...
        className="ml-4 py-2 px-4 bg-green-500 text-text rounded hover:bg-green-700"
      >
        Top 3 を選択
      </button>
      <div className="flex gap-2 flex-wrap mb-4">
        {top_teches.map((tech, index) => (
          <span key={tech} className="bg-secondary text-text rounded-full px-4 py-1">
            {index + 1}位. {tech}
          </span>
        ))}
      </div>
      <div className="flex gap-2 flex-wrap mb-4">
        {selectedTech.map((tech) => (
          <span key={tech} className="bg-primary text-text rounded-full px-4 py-1">
            {tech}
          </span>
        ))}
      </div>
