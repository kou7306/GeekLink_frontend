...
type ContributionResult = number | number[];
...
// 1ヶ月のコントリビューション情報を取得する関数
export const getMonthlyContributionInfo = async (
  uuid: string
): Promise<{ logs: any[] }> => {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    ...
    const time: number = 720;

    // キャッシュがないか有効期限が切れている場合、新たにデータを取得
    const response = await fetch(
      `${apiUrl}/activity/github?uuid=${uuid}&time=${time}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    localStorage.setItem(
      `contribution-info-last-update-${uuid}`,
      now.toString()
    );
    return contributions;
  } catch (error) {
...
// 以前の更新日時を持ち、差分のコントリビューションを取得する関数
type ContributionInfoResult = { logs: any[] };

export const getContributionsInfoSinceLastUpdate = async (
  uuid: string
): Promise<ContributionInfoResult> => {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    const lastUpdateKey = `contribution-info-last-update-${uuid}`;
    const now = new Date();
